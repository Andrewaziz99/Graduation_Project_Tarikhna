    ...
    const deleteLesson = async (lessonID) => {
        ...
        try {
            if (props.type == 'question') {
                const response = await fetch('http://localhost:9000/questions/deleteQuestion/?questionID=' + lessonID, {
                    method: "DELETE"
                })
                ...
                const response = await fetch('http://localhost:9000/lesson/deleteLesson/?id=' + lessonID, {
                    method: "DELETE"
                })
                const data = await response.json()
                if (data.status) {
